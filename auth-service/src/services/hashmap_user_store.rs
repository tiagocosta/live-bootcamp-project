...
mod tests {
    ...
    async fn test_validate_user() {
        ...
        let result = store.add_user(user.clone()).await;
        assert!(result.is_ok());
        let result = store.validate_user("test@example.com", "password").await;
        assert!(result.is_ok());
        let result = store.validate_user("test@example.com", "wrong_password").await;
        assert!(result.is_err());
        assert_eq!(result.err().unwrap(), UserStoreError::InvalidCredentials);
        let result = store.validate_user("nonexistent@example.com", "password").await;
        assert!(result.is_err());
        assert_eq!(result.err().unwrap(), UserStoreError::UserNotFound);
    }
}